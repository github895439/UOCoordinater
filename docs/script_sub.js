...
window.addEventListener("load", function () {
    let querys = decodeURI(document.URL).split("?").pop().split("&");
    let row = "";
    let col = "";
    let value = "";
    ...
    document.querySelector("#row").innerText = row;
    document.querySelector("#col").innerText = col;
    document.querySelector("#value").innerText = value;
    return;
});

function btnHandlerDebug(params) {
    window.open("./sub.html", "sub_UOCoordinater");
    alert(window.sub_UOCoordinater.document)
    // importCsvNameCrdnts = {
